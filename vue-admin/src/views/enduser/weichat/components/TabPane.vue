  ...
  <el-table :data="list" border fit highlight-current-row
    ...
    <el-table-column width="130px" label="账号名称" prop="accountName" show-overflow-tooltip="true">
      <template slot-scope="scope">
        <span> {{scope.row.accountName | decodeName}}</span>
      </template>
    </el-table-column>
    ...
    <el-dialog title="账号详情" :visible.sync="dialogFormVisible">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="账号名称" prop="accountName">
          <el-input disabled v-model="deAccName">
          </el-input>
        </el-form-item>
        </el-col>
...
export default {
  ...
  computed: {
    deAccName:{
      get: function () {
        return Base64.decode(this.form.accountName)
      },
        set: function (value) {
        this.form.accountName = Base64.encode(value);
      }
    }
  ...
  methods: {
    ...
    getList(page) {
      ...
      getAccounts(page,2,this.listQuery.type).then(res => {
